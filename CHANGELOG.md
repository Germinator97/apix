...
* **`ErrorMapperInterceptor` no longer replaces an already-typed exception.**
  ...
  `OutOfStockException` had it rewritten to
...
production app onto 3.0.0. Eight gaps, none of which broke a build — three of them
...
* **⚠️ BREAKING BEHAVIOR — Retry is now HTTP-method-aware; `POST` and `PATCH` are no longer retried by default**
  - Automatic retry previously replayed **any** request whose status code matched `retryStatusCodes`, ignoring the HTTP method. A `5xx` returned *after* the server had already committed (e.g. a gateway `502`/`504` timeout following an order) would replay a non-idempotent request and produce a **duplicate** (the same order placed twice).
