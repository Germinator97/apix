...
Many public APIs accept an `Idempotency-Key` header. For any backend with non-idempotent writes this is critical: a write replayed by a retry happens twice. Today, developers must remember to set this header manually on every write call. ApiX should make it ergonomic.
...
> idempotency key was the *only* thing standing between a `502` and a duplicate
> write, and "the key survives the retry" was the whole point.
>
