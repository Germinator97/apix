...
1. **Given** ErrorTrackingConfig with environment and onError callback
...
- [x] Task 1: Create ErrorTrackingConfig
  - [x] enabled flag
  - [x] environment parameter
  - [x] onError callback
  - [x] onBreadcrumb callback
  - [x] captureStatusCodes set (default: 5xx)
  - [x] Header redaction
  - [x] Body truncation

- [x] Task 2: Create ErrorTrackingInterceptor
...
- [x] Task 3: Write unit tests
  - [x] ErrorTrackingConfig tests
  - [x] HttpTrackingException tests
  - [x] ErrorTrackingInterceptor tests (17 total)
...
```dart
// With Sentry SDK
final interceptor = ErrorTrackingInterceptor(
...
- HttpTrackingException for HTTP errors
- Request/response breadcrumbs
- 17 tests

### File List

- `lib/src/observability/error_tracking_interceptor.dart` - Interceptor + Config
- `lib/src/observability/sentry_setup.dart` - SentryFlutter initialization
- `lib/apix.dart` - Export
- `test/observability/error_tracking_interceptor_test.dart` - 17 tests
- `pubspec.yaml` - Added sentry_flutter dependency

### Change Log

- 2026-03-17: Story 7.3 implemented - ErrorTrackingInterceptor
