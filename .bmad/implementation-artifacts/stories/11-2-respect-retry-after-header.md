...
Today `retry_interceptor.dart:65` uses `config.getDelay(currentAttempt)` (exponential backoff) regardless of headers. When a backend explicitly sends `Retry-After: 60`, we still retry after ~1s. This is a real bug for endpoints that ban clients ignoring rate limits.
