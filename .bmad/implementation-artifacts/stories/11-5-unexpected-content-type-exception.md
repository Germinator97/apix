...
Some APIs return `text/plain` for valid JSON, or omit `Content-Type`. Forcing strict mode by default would break existing integrations. Opt-in keeps backward compat and lets developers tighten security where it matters.
