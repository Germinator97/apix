  ...
  group('M8 — a business failure dressed as 200 is treated as a failure', () {
    ...
    test('a valid body still passes, and is still cached', () async {
      final adapter = ScriptedAdapter(
        (options, i) => jsonResponse({'success': true, 'count': 42}, 200),
      );
      final measured = <RequestMetrics>[];
      final client = ApiClientFactory.create(
        baseUrl: 'https://api.test',
      ...
      final first = await client.get<dynamic>('/orders');
      final second = await client.get<dynamic>('/orders');

      expect(bodyOf(first)['count'], 42);
