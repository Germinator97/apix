...
CacheEntry _entry({
  String data = '{"email":"jane@example.test","plan":"premium"}',
...
void main() {
  ...
  group('EncryptedCacheStorage', () {
    test('round-trips a body unchanged', () async {
      ...
      await storage.set('GET:/profile', original);

      final read = await storage.get('GET:/profile');
    ...
    test('what reaches the delegate is not the plaintext', () async {
      await storage.set('GET:/profile', _entry());

      final stored = await delegate.get('GET:/profile');

      expect(stored, isNotNull);
      expect(stored!.data, isNot(contains('jane@example.test')));
      expect(stored.data, isNot(contains('email')));
      expect(stored.data, isNot(contains('premium')));
    });
    ...
    group('unreadable entries', () {
      test('a wrong key reads as a miss, not an exception', () async {
        ...
        expect(await wrongKey.get('GET:/profile'), isNull);
      });

      test('a corrupted entry reads as a miss', () async {
        await delegate.set('GET:/profile', _entry(data: 'not-sealed-at-all'));

        expect(await storage.get('GET:/profile'), isNull);
      });
      ...
      test('an unreadable entry is purged on read', () async {
        await delegate.set('GET:/profile', _entry(data: 'garbage'));

        await storage.get('GET:/profile');

        expect(await delegate.get('GET:/profile'), isNull);
      });
      ...
      test('has() also purges what it finds unreadable', () async {
        ...
        await storage.has('GET:/profile');

        expect(
          await delegate.get('GET:/profile'),
          isNull,
    ...
    group('the invalidation API keeps working', () {
      setUp(() async {
        ...
        await storage.set('GET:https://api.test/profile', _entry());
      });
      ...
      test('keys are readable, so they can be matched', () async {
        ...
        expect(
          await storage.keys(),
          contains('GET:https://api.test/profile'),
      ...
      test('removeWhere still matches', () async {
        expect(
          await storage.removeWhere((k) => k.contains('/profile')),
          equals(1),
        );
      });

      test('remove and clear pass through', () async {
        await storage.remove('GET:https://api.test/profile');
