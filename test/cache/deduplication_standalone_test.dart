...
void main() {
  group('standalone deduplication', () {
    ...
    test('leaves non-listed methods alone', () async {
      ...
      await Future.wait([
        client.post<dynamic>('/orders', data: {'quantity': 2}),
        client.post<dynamic>('/orders', data: {'quantity': 2}),
      ]);
      ...
      expect(
        adapter.hits,
        equals(2),
        reason: 'collapsing two identical POSTs would drop an order the '
            'caller meant to place twice',
      );
    });
  ...
  group('networkOnly never writes', () {
    ...
    // on only the first would leave profiles flowing into a store on exactly
